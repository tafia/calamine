...
fn table() {
    ...
    let table  = xls.table_by_name("Temperature").expect("Should exist").expect("Should not error");
    assert_eq!(table.name(), "Temperature");
    assert_eq!(table.columns()[0], "label");
    assert_eq!(table.columns()[1], "value");
    assert_eq!(table.data().get((0,0)), Some(&String("celsius".to_owned())));
    assert_eq!(table.data().get((1,0)), Some(&String("fahrenheit".to_owned())));
