...
fn test_region_columns(
    ...
    let full = full_wb.worksheet_range_region_ref(sheet, .., ..).unwrap();
    let mut proj_wb = open_workbook_auto(&path).expect(&path);
    let proj = proj_wb.worksheet_range_region_ref(sheet, cols, ..).unwrap();
...
fn test_region_full_equals_plain() {
    ...
    let full = x.worksheet_range_region_ref("Sheet1", .., ..).unwrap();
...
fn test_region_header_row_always_retained() {
    ...
    let range = x
        ...
        .worksheet_range_region_ref("Sheet1", .., 9..10)
        .unwrap();
...
fn test_region_through_sheets_auto() {
    ...
    let dispatched = sheets
        .worksheet_range_region_ref("Sheet1", [0, 2], ..)
        .unwrap();
...
fn test_region_unsupported_for_xls_ods() {
    for file in ["any_sheets.xls", "any_sheets.ods"] {
        ...
        assert!(
            matches!(
                sheets.worksheet_range_region_ref(&name, [0], ..),
