...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for (i, cf) in cfs.iter().enumerate() {
        println!("Block {}: range \"{}\" ({} rule(s))", i + 1, cf.sqref, cf.rules.len());
        ...
        for rule in &cf.rules {
            ...
            match &rule.rule_type {
                ...
                ConditionalFormatRuleType::ColorScale2 {
                    min, max, min_color, max_color,
                } => {
                    println!(
                        "2-Color Scale: {:?}({:?}) {} -> {:?}({:?}) {}",
                        min.value_type, min.value, min_color,
                        max.value_type, max.value, max_color,
                    );
                }
                ConditionalFormatRuleType::ColorScale3 {
                    min, mid, max, min_color, mid_color, max_color,
                } => {
                    println!(
                        "3-Color Scale: {:?}({:?}) {} -> {:?}({:?}) {} -> {:?}({:?}) {}",
                        min.value_type, min.value, min_color,
                        mid.value_type, mid.value, mid_color,
                        max.value_type, max.value, max_color,
                    );
                }
                ConditionalFormatRuleType::DataBar {
                    ...
                    println!(
                        "DataBar: {:?}({:?}) -> {:?}({:?}), color={:?}",
                        min.value_type, min.value,
                        max.value_type, max.value,
                        fill_color,
                    );
                }
                ConditionalFormatRuleType::IconSet {
                    icon_type, thresholds, reversed, show_value,
                } => {
                    println!(
                        "IconSet {:?} ({} thresholds, reversed={}, show_value={})",
                        icon_type, thresholds.len(), reversed, show_value,
                    );
                }
                ...
                ConditionalFormatRuleType::AboveAverage {
                    above_average, equal_average, std_dev,
                } => {
                ...
                ConditionalFormatRuleType::NotContainsBlanks { .. } => println!("NotContainsBlanks"),
                ConditionalFormatRuleType::ContainsErrors { .. } => println!("ContainsErrors"),
                ConditionalFormatRuleType::NotContainsErrors { .. } => println!("NotContainsErrors"),
