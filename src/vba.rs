...
impl VbaProject {
    ...
    pub fn new<RS: Read + Seek>(r: &mut RS) -> Result<VbaProject, VbaError> {
        VbaProject::from_cfb(r)
    }

    /// Creates a new `VbaProject` out of a Compound File Binary and the corresponding reader
    pub fn from_cfb<RS: Read + Seek>(r: &mut RS) -> Result<VbaProject, VbaError> {
        // Read the VBA/dir stream in the vbaProject cfb file.
        let mut cfb = CompoundFile::open(r)?;
        let mut stream = cfb.open_stream("VBA/dir")?;
        ...
        for m in mods {
            let path = format!("VBA/{}", m.stream_name);
